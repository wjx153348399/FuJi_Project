...
class StationConfigTest(unittest.TestCase):
    def test_json_mode_uses_scan_targets_without_repository(self):
        repository = FakeRepository(
            targets=[ScanTargetConfig(flow="A050", dir="db-dir")]
        )
        ...
        self.assertEqual(effective.scan.targets[0].flow, "A010")
    ...
    def test_duplicate_directory_config_raises_clear_error(self):
        repository = FakeRepository(
            targets=[
                ScanTargetConfig(flow="A010", dir="same-dir"),
                ScanTargetConfig(flow="A050", dir="same-dir"),
            ]
        )
    ...
    def test_absolute_directory_config_raises_clear_error(self):
        repository = FakeRepository(
            targets=[ScanTargetConfig(flow="A010", dir=r"C:\share\station")]
        )

        with self.assertRaisesRegex(ConfigError, "relative to share.root"):
            build_effective_scan_config(_config(source="db"), repository)

    def test_invalid_flow_code_raises_clear_error(self):
        repository = FakeRepository(
            targets=[ScanTargetConfig(flow="AFC", dir="station-a")]
        )

        with self.assertRaisesRegex(ConfigError, "station code like A032"):
            build_effective_scan_config(_config(source="db"), repository)
...
def _config(source: str) -> AppConfig:
    return AppConfig(
        ...
        scan=ScanConfig(
            ...
            targets=[ScanTargetConfig(flow="A010", dir="json-dir")],
        ),
        watch=WatchConfig(),
